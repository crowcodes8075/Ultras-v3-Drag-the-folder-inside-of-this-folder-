...
public class CoreBots2 : CoreBots
{
...
public class CoreEnhancements
{
    ...
    )
    {
        ...
        if (!weaponAlreadyEnhanced && weapon != null && weapon.EnhancementLevel == Bot.Player.Level)
        {
            C.Logger($"[CoreEnhancements] Weapon early-check mismatch: \"{weapon.Name}\" Pattern={weapon.EnhancementPatternID} Proc={getProcID(weapon)} Level={weapon.EnhancementLevel} ExpectedPattern={((int)wSpecial <= 6 ? (int)type : 10)} ExpectedProc={(int)wSpecial}");
        }
