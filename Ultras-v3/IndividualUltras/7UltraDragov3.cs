...
public class UltraDrago_v3
{
    ...
    private void Prep()
    {
        ...
        C.Logger($"[UltraDrago-v3] Taunt offset: {tauntOffsetSeconds}s for class: {className}");
    }
    ...
    private void Fight()
    {
        ...
        const string drop = "King Drago Insignia";
        const string leftSummon = "Executioner Dene";
        const string rightSummon = "Bowmaster Algie";
        const string BossDefeated = "King Drago Defeated";
        ...
        while (!Bot.ShouldExit)
        {
            ...
            if (IsTaunter() && Bot.Player.HasTarget)
            {
                ...
                if (offsetTime <= TauntWindowSeconds && (DateTime.UtcNow - lastTauntTime).TotalSeconds >= TauntIntervalSeconds - 1)
                {
                    ...
                    C.Logger($"[UltraDrago-v3] Taunt window active at {elapsed:F1}s — taunting {leftSummon}.");
                    _ = DragoTauntAsync(leftSummon);
                }
            }
    ...
    private async Task DragoTauntAsync(string targetSummon)
    {
        for (int i = 0; i < 60 && !Bot.ShouldExit; i++)
        {
            ...
            if (!Bot.Player.HasTarget || Bot.Player.Target?.Name != targetSummon)
                Bot.Combat.Attack(targetSummon);

            Engine.Cast(5);
            await Task.Delay(50);
        }

        C.Logger("[UltraDrago-v3] Taunt done — 60 presses complete.");
    }
}
