...
description: Ultra Nulgath v3 — timer-based taunt rotation with blade priority targeting and army sync.
*/
...
public class UltraNulgath_v3
{
    ...
    // DPS classes
    private const string Dps1 = "ArchPaladin";
    ...
    private void Prep()
    {
        ...
        C.Logger($"[UltraNulgath-v3] Taunt offset: {tauntOffsetSeconds}s for class: {className}");
    }
    ...
    private void Fight()
    {
        ...
        const string drop = "Nulgath Insignia";
        const string BossDefeated = "Nulgath the Archfiend Defeated?";
        ...
        while (!Bot.ShouldExit)
        {
            ...
            // ── Dynamic targeting: taunters focus Nulgath (2), DPS focus Blade (1) then Nulgath ──
            if (IsTaunter())
            {
            ...
            if (IsTaunter() && Bot.Player.HasTarget)
            {
                ...
                if (offsetTime <= TauntWindowSeconds && (DateTime.UtcNow - lastTauntTime).TotalSeconds >= TauntIntervalSeconds - 1)
                {
                    ...
                    C.Logger($"[UltraNulgath-v3] Taunt window active at {elapsed:F1}s.");
                    ...
                    C.Logger("[UltraNulgath-v3] Taunt done — 60 presses complete.");
                }
            }
