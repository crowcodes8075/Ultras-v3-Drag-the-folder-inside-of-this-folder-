...
public class UltraWarden_v3
{
    ...
    private void Fight()
    {
        ...
        while (!Bot.ShouldExit)
        {
            ...
            {
                C.Logger("Ultra Warden defeated. Stopping skills immediately.");

                // ── Cancel taunt thread, skills stop automatically via DisableSkills in RunBoss() finally ──
                _tauntCts.Cancel();

                C.Logger("Skills stopped. Waiting 30s for army completion.");
                Engine.Join(map);
                Ultra.JoinHouse();
