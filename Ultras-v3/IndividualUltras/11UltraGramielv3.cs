...
description: Ultra Gramiel v3 — two-phase fight (crystals then Gramiel) with chat-driven crystal taunt warnings and timer-based taunt rotation.
*/
...
public class UltraGramiel_v3
{
    ...
    private const string RightCrystalT1 = "Lord Of Order";
    private const string LeftCrystalT2 = "ArchPaladin";
    ...
    // Gramiel phase timing: 4 taunters x 4s = 16s full cycle
    private const double GramielTauntIntervalSeconds = 16.0;
    private const double GramielTauntWindowSeconds = 4.0;

    private DateTime fightStartTime = DateTime.MinValue;
    private DateTime lastGramielTauntTime = DateTime.MinValue;
    private DateTime gramielFightStartTime = DateTime.MinValue;
    private double gramielTauntOffsetSeconds = 0;
    ...
    private void ApplyTaunterClasses()
    {
        ...
        if (className.Equals(LeftCrystalT1, StringComparison.OrdinalIgnoreCase))
        {
            ...
            C.Logger($"[UltraGramiel-v3] Assigned to taunt LEFT crystal (mapId=2) - T1");
        }
        else if (className.Equals(LeftCrystalT2, StringComparison.OrdinalIgnoreCase))
        {
            ...
            isTaunter = true;
            isT1Taunter = false;
            gramielTauntOffsetSeconds = 4;
            C.Logger($"[UltraGramiel-v3] Assigned to taunt LEFT crystal (mapId=2) - T2");
        }
        else if (className.Equals(RightCrystalT1, StringComparison.OrdinalIgnoreCase))
        {
            ...
            isTaunter = true;
            isT1Taunter = true;
            gramielTauntOffsetSeconds = 8;
            C.Logger($"[UltraGramiel-v3] Assigned to taunt RIGHT crystal (mapId=3) - T1");
        }
        else if (className.Equals(RightCrystalT2, StringComparison.OrdinalIgnoreCase))
        {
            ...
            isTaunter = true;
            isT1Taunter = false;
            gramielTauntOffsetSeconds = 12;
            C.Logger($"[UltraGramiel-v3] Assigned to taunt RIGHT crystal (mapId=3) - T2");
        }
        else
        {
            crystalMapId = 2;
    ...
    private void EquipPresetClasses()
    {
        ...
        Ultra.EquipClassSync(classSlots, armySize, "ultra_gramiel_class-v3.sync", allowDuplicates);
    }
    ...
    private void Prep()
    {
        ...
        if (skipThird)
        {
            C.Logger("[UltraGramiel-v3] Taunter detected, acquiring Scroll of Enrage after potion setup.");
    ...
    private void Fight()
    {
        ...
        const string drop = "Gramiel the Graceful Vanquished";
        const string BossDefeated = "Gramiel the Graceful Defeated";

        const string waitSyncFile = "ultra_gramiel.sync";
        const string wipeDeadSyncFile = "UltraGramielWipeDead.sync";
        const string wipeAliveSyncFile = "UltraGramielWipeAlive.sync";
        ...
        while (!Bot.ShouldExit)
        {
            ...
            if (UltraGeneral.ArmyWipeHelperWithTaunters(Ultra, Bot, wipeDeadSyncFile, wipeAliveSyncFile, ref armyWipeDetected, ref fightStartTime, ref lastGramielTauntTime))
                continue;
            ...
            // Completion check: boss no longer alive
            ...
            {
                gramielFightStartTime = DateTime.UtcNow;
                C.Logger("[UltraGramiel-v3] All crystals dead. Beginning Gramiel taunt rotation.");
            }
            ...
            {
                double elapsed = (DateTime.UtcNow - gramielFightStartTime).TotalSeconds;
                double gramielTimeInCycle = (elapsed - gramielTauntOffsetSeconds) % GramielTauntIntervalSeconds;
                ...
                bool gramielCooldownExpired = (DateTime.UtcNow - lastGramielTauntTime).TotalSeconds >= GramielTauntIntervalSeconds - 1;

                if (gramielInTauntWindow && gramielCooldownExpired)
                {
                    lastGramielTauntTime = DateTime.UtcNow;
                    C.Logger($"[UltraGramiel-v3] Gramiel taunt window active at {elapsed:F1}s.");
                    _ = GramielTauntAsync();
                }
            }
    ...
    private async Task GramielTauntAsync()
    {
        for (int i = 0; i < 60 && !Bot.ShouldExit; i++)
        {
            if (!Bot.Player.Alive)
        ...
        C.Logger("[UltraGramiel-v3] GramielTauntAsync done — 60 presses complete.");
    }
